<template>
	<div class="home">
		<div class="select-box" style="display: inline-block">
			<label>选市区：</label>
			<a-select placeholder="请选择市区" style="width: 240px" v-model="province">
				<a-select-option
					:value="item.value"
		...
		<div class="select-box" style="display: inline-block;margin-left: 20px;">
			<label>选择城市：</label>
			<a-select placeholder="请选择城市" style="width: 240px" v-model="defaultCity" @change="cityChangeFn">
				<a-select-option :value="item.value" v-for="(item,index) in cityArr" :key="index">{{item.name}}</a-select-option>
			</a-select>
		</div>
...
import cityAllArray from "@/utils/cityData";
...
export default {
	...
	watch: {
		province(data) {
			if (data == "hz") {
				this.cityArr = cityAllArray[0];
			} else if (data == "fuzhou") {
				this.cityArr = cityAllArray[1];
			}
			this.defaultCity = this.cityArr[0].value;
			this.getNewsList();
		}
	},
	data() {
